...
testDirectory = '../data/12Oct2020/Accuracy/'; %Data directory
imList = dir(fullfile(testDirectory,'accuracy2*')); %List of images
worldProbePoints = generateCheckerboardPoints([4,5], 5); 
...
for i = 1:length(imList)
    ...
    %Find the probe
    [Pcam, boardSize, imagesUsed] = detectCheckerboardPoints(thisIm);
    if ~isempty(Pcam)
        %Get the camera pose
...
%Plotting
figure
scatter3(worldCoords(:,1),worldCoords(:,2),worldCoords(:,3),'o')
hold on
...
figure
plot(trueDistFromStart,measDistFromStart,'o')
hold on
plot([0,210],[0,210],'k--')
