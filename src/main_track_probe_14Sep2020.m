...
data_dir = '../data/14Sep2020/raw';

% Load calibration camera params
%[file,path] = uigetfile('*.mat','Choose cameraParams.mat');
%load(fullfile(path, file))
%
intmat = readNPY('cameraMatrix.npy');
distCoefs = readNPY('distortionCoefs.npy');
...
%%
%Get list of images
D = dir([data_dir, '/accuracyHoriz_frame*.png']);
nframes = length(D);
%Get the frame
frame = imread(fullfile(D(1).folder,D(1).name));
file = D(1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:nframes
  ...
  %file
  thisIm = imread(fullfile(D(i).folder,D(i).name));
  %Convert frame to grayscale
  frame = rgb2gray(im2double(thisIm));
  try
     %Get probe location
    [newCameraParams, board_rect, checkerboard_pts] = ...
      track_probe_mba(squareSize, frame, cameraParams, board_rect, prevCameraParams, theta_axis);
...
fullphantom = imread(fullfile(file.folder, file.name));
